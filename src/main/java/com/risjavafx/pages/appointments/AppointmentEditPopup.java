...
public class AppointmentEditPopup implements Initializable {
    ...
    public static int clickedAppointmentId;

    Driver driver = new Driver();
    Miscellaneous misc = new Miscellaneous();

    public AppointmentEditPopup() throws SQLException {
    }
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        Popups.APPOINTMENT_EDIT.getPopup().showingProperty().addListener((observableValue, aBoolean, t1) -> {
            PageManager.getRoot().setDisable(!aBoolean);

            if (Popups.APPOINTMENT_EDIT.getPopup().isShowing()) {
                appointmentIdLabel.setText(String.valueOf(getAppointmentClickedId()));
                refreshElements();
            }
        });
    ...
    public static void setAppointmentClickedId(int appointmentClickedId) {
        AppointmentEditPopup.clickedAppointmentId = appointmentClickedId;
    }

    public static int getAppointmentClickedId() {
        return AppointmentEditPopup.clickedAppointmentId;
    }


    public void updateAppointmentQuery() throws SQLException {
        String sql = """
                UPDATE appointments
                SET patient = ?, modality = ?, date_time = ?, radiologist = ?, technician = ?, phone_number = ?, email_address = ?
                WHERE appointments.appointment_id = ?;
                """;


        PreparedStatement preparedStatement = driver.connection.prepareStatement(sql);
        preparedStatement.setInt(1, pullPatientComboboxId(roleComboBoxPatient.getValue()));
        preparedStatement.setInt(2, pullModalityComboboxId(roleComboBoxModality.getValue()));
        preparedStatement.setString(3, dateTextField.getText());
        preparedStatement.setInt(4, pullDocComboboxId(roleComboBoxRad.getValue()));
        preparedStatement.setInt(5, pullDocComboboxId(roleComboBoxTech.getValue()));
        preparedStatement.setString(6, phoneNumberTextField.getText());
        preparedStatement.setString(7, emailTextField.getText());
        preparedStatement.setInt(8, getAppointmentClickedId());
